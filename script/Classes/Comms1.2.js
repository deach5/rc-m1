...
Comms.prototype.connectAPI = function (_runtime) {
  ...
  if (_runtime == "scorm") {
    ...
    while (tries < triesMax && api == null) {
      ...
      if (api == null && typeof window.parent != "undefined")
        api = this.findAPI(window.parent);
      if (api == null && typeof window.top != "undefined")
        api = this.findAPI(window.top);
      if (api == null && typeof window.opener != "undefined")
    ...
    if (_runtime == "local") {
      ...
      this.throwError(
        "<p>Unknown runtime argument <b>'" +
        _runtime +
        "'</b> passed to <b>connectAPI</b> in <b>Comms class</b>. Expected scorm, local or dummy.</p>"
      );
      return;
    }
  }
...
Comms.prototype.findAPI = function (win) {
  ...
  if (win.frames.length > 0) {
    for (var i = 0; i < win.frames.length; i++) {
      if (
        typeof win.frames[i] != "undefined" ?
        typeof win.frames[i].API != "undefined" :
        false
      ) {
...
Comms.prototype.commsError = function (_error, _data1, _data2) {
  ...
  var msg, api;
  _msg =
    "<p>ERROR: The module has failed to make a connection to the Learning Management System. This may be due to a network dropout.</p><br>";
  api = this.getAPI();
  ...
  msg +=
    "<br><p>Your progress will not be tracked. Please exit the module and re-launch it again. </p><p><b>If the problem persists, please contact you system administrator.</b></p><p>Please note down the error message above as it will be of use to determine the fault.</p>";
  this.throwError(msg);
};
// PRIVATE
